...
export function writeImageIndexed(pixels, width, height) {
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    dataView.setUint8(
      HEADER_SIZE + colorMapSize + i * bytePerPixel,
      foundColorIndex
    );
  }
...
export function writeImageAutoDetectBestFormat(pixels, width, height) {
  ...
  if (
    indexedColorImage != undefined &&
    indexedColorImage.byteLength < trueColorImage.byteLength
  ) {
...
export function readImage(dataBuffer) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      let alpha = 0xff;
      if (colorMap) {
        let colorId = dataView.getUint8(
          headerSize + paletteSize + y * width + x
        );
        ...
        const pixelPosition =
          headerSize + y * bytePerPixel * width + x * bytePerPixel;
        if (bytePerPixel == 4) {
